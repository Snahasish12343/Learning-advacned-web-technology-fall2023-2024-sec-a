...
const SubmitForm = () => {
  ...
  const handleCreditUpdate = async (operation: 'add' | 'subtract') => {
    try {
      ...
      if (response.data) {
        ...
        const updatedCredit = operation === 'add' ? currentCredit + 10 : currentCredit - 10;
  ...
  return (
    <div>
      ...
      <form>
        ...
        <div>
          <button type="button" onClick={() => handleCreditUpdate('add')}>Add 10</button>
          <button type="button" onClick={() => handleCreditUpdate('subtract')}>Subtract 10</button>
        </div>
      </form>
    </div>
  );
